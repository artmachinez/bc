(function ($) {
    ...
    $.fn.counter.getDataLoop = function (timeout, currentVisBox, totalVisBox, IPBox) {
        $.ajax({
            type: 'POST',
            url: 'modules/{{_setup.id}}/WebService.asmx/GetData',
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function (data) {
                currentVisBox.text(data.d.currentVisitors);
                totalVisBox.text(data.d.totalVisitors);
                IPBox.text(data.d.requestIP);
